...
class AdminVideoFetchController extends Controller
{
  ...
  public function index()
    {
        $videos = Video::where('type','=',1)->orderBy('id','desc')->get();
        return view('admin.videofetch.index',compact('videos'));
    }


    public function create()
    {
        return view('admin.videofetch.create',compact('cats'));
    }
  ...
  public function status($id1,$id2)
    {
        ...
        return redirect()->route('admin-fetch-index');
    }
...
//If the url has youtube then execute the if statement
            ...
            if (strpos($url, 'youtube') !== false) {
                ...
                $queryString = parse_url($url, PHP_URL_QUERY);
                    ...
                    return redirect()->route('admin-fetch-create')->with('unsuccess','The URL is Invaild.');;
                    }
                $api_url = 'https://www.googleapis.com/youtube/v3/videos?key='.$api_key.'&fields=items(snippet(title,description,tags,thumbnails))&part=snippet&id='.$id;                        
                 ...
                 $link = "youtube";
                 // dd($fetch);
                 return view('admin.videofetch.createfetch',compact('link','fetch','id','url'));
...
//If the url has youtube then execute the if statement
            ...
            if (strpos($url, 'youtube') !== false) {
                ...
                $api_url = 'https://www.googleapis.com/youtube/v3/videos?key='.$api_key.'&fields=items(snippet(title,description,tags,thumbnails))&part=snippet&id='.$id;                        
                 ...
                 $link = "youtube";
                 // dd($fetch);
                 return view('admin.videofetch.edit',compact('link','fetch','id','url','video'));
...
//Otherwise return with an error message
        ...
        {
        return redirect()->route('admin-fetch-edit',['id'=>$video->id])->with('unsuccess','The URL is Invaild.');
        }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin-fetch-index')->with('success','New Video Added Successfully.');
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.videofetch.edit',compact('video','cats','tags','ext','id'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $video->update($data);
        return redirect()->route('admin-fetch-index')->with('success','Video Updated Successfully.');
    }
    ...
    public function destroy($id)
    {
        ...
        $video->delete();
        return redirect()->route('admin-fetch-index')->with('success','Video Deleted Successfully.');
    }
}
